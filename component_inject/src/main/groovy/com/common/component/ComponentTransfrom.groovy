...
public class ComponentTransfrom extends Transform {

    private Project project
    ClassPool classPool
    String applicationName

    ComponentTransfrom(Project project) {
        this.project = project
    }

    @Override
    String getName() {
        return "ComponentTransfrom"
    }
    ...
    private boolean isActivator(CtClass ctClass) {
        try {
            for (CtClass ctClassInter : ctClass.getInterfaces()) {
                if (project.extensions.combuild.applicationLikeName.equals(ctClassInter.name)) {
                    return true;
                }
            }
        } catch (Exception e) {
